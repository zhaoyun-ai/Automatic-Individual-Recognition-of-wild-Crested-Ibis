...
def _one_linear_eval(X, y, X_val, y_val, X_test, y_test, hidden_sizes, epochs, early_stopping, debug):
    ...
    mlp = MLPClassifier(hidden_sizes, max_iter=1000, alpha=0.0001, learning_rate_init=0.01,
                        activation='logistic')
    mlp.fit(X, y)

    score = metrics.roc_auc_score(y_test, mlp.predict_proba(X_test), multi_class='ovo')
    return score
