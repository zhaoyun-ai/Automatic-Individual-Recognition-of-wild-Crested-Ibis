...
def error(cluster, target_cluster, k):
    ...
    return acc,pred_corresp
numpy_array = np.load('BYOLA-3072-test-feature.npy')
labels = np.load('BYOLA-3072-test-label.npy')
...
#tsne = TSNE()
#clusterable_embedding = tsne.fit_transform(numpy_array)
clusterer = hdbscan.HDBSCAN(min_samples=10,min_cluster_size=120)
...
clusterer.fit(clusterable_embedding)
#clusterer.labels_=clusterer.predict(clusterable_embedding)
#clusterer.labels_= clusterer.predict(numpy_array)
#clusterer.fit(clusterable_embedding)
#x=error(labels,clusterer.labels_,10)
x,b=error(clusterer.labels_,labels,10)
print("正确率:",x)
...
scatter_classes= plt.scatter(clusterable_embedding[:, 0], clusterable_embedding[:, 1],c=labels,  s=0.25, cmap='Spectral')
plt.legend(*scatter_classes.legend_elements(),
           loc="upper right", title="true labels")
import matplotlib.pyplot as plt
