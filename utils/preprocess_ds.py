...
def convert_fsdnoisy_metadata(root):
    FSD = Path(root)
    train_df = pd.read_csv(FSD / f'meta/train.csv')
    # train_df = train_df[train_df.manually_verified != 0]
    # train_df = train_df[train_df.noisy_small == 0]
    test_df = pd.read_csv(FSD / f'meta/test.csv')
    # fname := split/fname
    train_df['fname'] = 'chiffchaff-train/' + train_df.fname
    test_df['fname'] = 'chiffchaff-test/' + test_df.fname
    # split. train -> train + val
    train_df['split'] = 'train'
    valid_index = np.random.choice(train_df.index.values, int(len(train_df) * 0.1), replace=False)
    train_df.loc[valid_index, 'split'] = 'valid'
    ...
    # filename -> file_name
    df.columns = [c if c != 'fname' else 'file_name' for c in df.columns]
    df.to_csv(f'work/metadata/chiffchaff.csv', index=False)
    n_samples = len(df)

    df = pd.read_csv(f'work/metadata/chiffchaff.csv')
    assert len(df) == n_samples, f'{len(df)}'
    print(f'Created work/metadata/fsdnoisy18k.csv - test passed')
...
def chiffchaff(root):
    convert_fsdnoisy_metadata(root)
    convert_wav(root, 'work/16k/fsdnoisy18k', verbose=False)
